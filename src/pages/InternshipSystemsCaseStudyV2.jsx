...
function CheckDot() {
  return <span aria-hidden="true" className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-neutral-100" />;
}
...
function SystemFrame({ system, reduced, onOpen, eager = false }) {
  ...
  return (
    <figure className="group relative overflow-hidden rounded-2xl border border-white/10 bg-neutral-900 shadow-[0_24px_80px_rgba(0,0,0,0.5)]">
      <span aria-hidden="true" className="pointer-events-none absolute left-4 top-4 z-20 h-6 w-6 border-l-2 border-t-2 border-neutral-100/40" />
      <span aria-hidden="true" className="pointer-events-none absolute bottom-4 right-4 z-20 h-6 w-6 border-b-2 border-r-2 border-neutral-100/40" />
      <div className="relative aspect-[16/10] overflow-hidden bg-black">
        ...
        <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-gradient-to-t from-neutral-950/70 via-transparent to-black/20" />
        <div className="absolute left-4 top-4 z-10">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-neutral-950/80 px-3 py-1 text-[0.6rem] font-semibold uppercase tracking-[0.16em] text-white backdrop-blur">
            <span className={["h-1.5 w-1.5 rounded-full", video && !reduced ? "animate-pulse bg-emerald-400" : "bg-neutral-600"].join(" ")} />
            {video ? "System demo" : "Preview"}
          </span>
        </div>
        {onOpen && (
          <button type="button" onClick={() => onOpen(system)} aria-label={`Expand ${system.name} preview`} className="absolute bottom-4 right-4 z-10 rounded-full border border-white/15 bg-neutral-950/80 px-3.5 py-2 text-[0.7rem] font-semibold text-white backdrop-blur transition hover:border-neutral-100/40 hover:text-white">
            Open demo ↗
          </button>
        )}
      </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
      {systems.slice(0, 4).map((system, i) => {
        const poster = system?.media?.cover ?? null;
        return (
          <motion.div key={system.id} initial={reduced ? false : { opacity: 0, scale: 0.93, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.65, delay: reduced ? 0 : 0.25 + i * 0.08, ease: EASE }} className="group relative overflow-hidden rounded-xl border border-white/10 bg-neutral-900 shadow-[0_12px_40px_rgba(0,0,0,0.45)]">
            {poster ? (
              ...
              <div className="flex aspect-video items-center justify-center bg-neutral-900/80">
                <span className="text-[0.55rem] font-semibold uppercase tracking-widest text-neutral-600">No preview</span>
              </div>
            )}
            <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-gradient-to-t from-neutral-950/80 via-transparent to-transparent" />
            <p className="absolute bottom-2 left-2.5 text-[0.6rem] font-semibold uppercase tracking-[0.14em] text-white/90">
        ...
        {steps.map((step, i) => {
          ...
          return (
            <motion.div key={step.id} initial={reduced ? false : { opacity: 0, x: i % 2 === 0 ? -28 : 28, filter: "blur(4px)" }} whileInView={reduced ? undefined : { opacity: 1, x: 0, filter: "blur(0px)" }} viewport={{ once: true, amount: 0.4 }} transition={{ duration: 0.6, delay, ease: EASE }} className={["relative flex", i % 2 === 0 ? "justify-start" : "justify-end"].join(" ")}>
              <div className={["relative w-[calc(50%-1.25rem)]", isSystem ? "rounded-2xl border border-neutral-100/25 bg-neutral-100/[0.06] p-4 shadow-[0_10px_32px_rgba(0,0,0,0.38)]" : "py-3 pl-2 pr-4"].join(" ")}>
                <span aria-hidden="true" className={["absolute top-1/2 -translate-y-1/2", i % 2 === 0 ? "-right-[1.35rem]" : "-left-[1.35rem]", "h-2.5 w-2.5 rounded-full border-2", isSystem ? "border-neutral-100 bg-neutral-100 shadow-[0_0_10px_rgba(255,255,255,0.22)]" : "border-neutral-600 bg-neutral-950"].join(" ")} />
                {isSystem ? (
                  <>
                    <p className="text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-neutral-100">System</p>
                    <p className="mt-1 text-sm font-semibold leading-tight text-white">{step.label}</p>
                    <p className="mt-1.5 text-xs leading-5 text-neutral-400">{step.description}</p>
                  </>
                ) : (
                  <>
                    <p className="text-[0.68rem] font-semibold text-neutral-400">{step.label}</p>
                    <p className="mt-0.5 text-[0.62rem] leading-5 text-neutral-600">{step.description}</p>
                  </>
                )}
              </div>
            </motion.div>
  ...
  return (
    <Reveal delay={delay} reduced={reduced}>
      <div className="flex gap-4 border-t border-white/[0.07] py-5 first:border-t-0 first:pt-0">
        <span className="w-28 shrink-0 pt-0.5 text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-neutral-500">{label}</span>
        <p className="text-sm leading-7 text-neutral-300">{value}</p>
      </div>
    </Reveal>
  );
}
  ...
  return (
    <Reveal delay={reduced ? 0 : index * 0.1} reduced={reduced}>
      <article className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025] p-6 sm:p-8">
        <motion.div aria-hidden="true" initial={reduced ? false : { scaleX: 0 }} whileInView={reduced ? undefined : { scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: reduced ? 0 : index * 0.1 + 0.2, ease: EASE }} className="absolute inset-x-0 top-0 h-px origin-left bg-gradient-to-r from-neutral-100/80 via-neutral-100/25 to-transparent" />
        <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.06),transparent_20rem)]" />
        <div className="relative">
          <div className="flex flex-wrap items-center gap-2.5">
            <span className="rounded-full border border-neutral-100/20 bg-neutral-100/[0.06] px-3 py-1.5 text-[0.6rem] font-semibold uppercase tracking-[0.18em] text-white">{challenge.system}</span>
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <div className="rounded-xl border border-rose-400/15 bg-rose-500/[0.03] p-4">
              ...
              <p className="mt-2.5 text-sm leading-7 text-neutral-400">{challenge.context}</p>
            </div>
            <div className="rounded-xl border border-emerald-400/15 bg-emerald-500/[0.03] p-4">
              ...
              <p className="mt-2.5 text-sm leading-7 text-neutral-400">{challenge.decision}</p>
            </div>
          </div>
        </div>
      </article>
    ...
    <Reveal delay={reduced ? 0 : index * 0.07} reduced={reduced}>
      <article className="overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.02] transition hover:border-white/15">
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex w-full items-center justify-between gap-4 p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:p-5"
        >
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[0.7rem] text-neutral-500">{system.tagline}</p>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-3">
            <div className="hidden flex-wrap gap-1.5 sm:flex">
              {(system.tech ?? []).slice(0, 3).map((t) => (
                <span key={t} className="rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[0.58rem] font-semibold uppercase tracking-[0.1em] text-neutral-400">
                  {t}
                </span>
              ))}
            </div>
            <svg viewBox="0 0 16 16" fill="none" aria-hidden="true" className={["h-4 w-4 shrink-0 text-neutral-400 transition duration-300", open ? "rotate-90" : ""].join(" ")}>
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    {(cs.problem || cs.solution) && (
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        {cs.solution && (
                          <div className="rounded-xl border border-neutral-100/15 bg-neutral-100/[0.04] p-4">
                            <p className="text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-neutral-100">
                              What Changed
                            </p>
                            <p className="mt-2.5 text-xs leading-6 text-neutral-400">
                              {cs.solution}
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-2">
                          {steps.map((step, i) => (
                            <span key={step} className="flex items-center gap-2">
                              <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-[0.68rem] font-medium text-neutral-300">
                                {step}
                              </span>
                              {i < steps.length - 1 && (
                                <span aria-hidden="true" className="font-mono text-[0.6rem] text-neutral-100/60">
                                  →
                                </span>
                              )}
                            </span>
                      ...
                      <div>
                        ...
                        <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                          {(system.keyFeatures ?? []).map((feat) => (
                            <li key={feat} className="flex items-start gap-2.5 text-xs leading-6 text-neutral-300">
                              <CheckDot />{feat}
                            </li>
                          ))}
                        </ul>
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        {cs.challenge && (
                          <div className="rounded-xl border border-amber-400/15 bg-amber-500/[0.025] p-4">
                            ...
                            <p className="mt-2 text-[0.72rem] font-semibold leading-5 text-neutral-200">
                              {cs.challenge.title}
                            </p>
                      ...
                      <div>
                        ...
                        <div className="mt-2.5 flex flex-wrap gap-2">
                          {(system.tech ?? []).map((t) => (
                            <span
                              key={t}
                              className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.025] px-2.5 py-1 text-[0.62rem] font-semibold text-neutral-400"
                            >
                              <span aria-hidden="true" className="h-1 w-1 rounded-full bg-neutral-100/60" />
                              {t}
                            </span>
                          ))}
                        </div>
    ...
    <CaseStudyLayout projectLabel="Internship Systems Case Study">
      <section className="mx-auto flex min-h-[70vh] w-full max-w-3xl items-center px-4 py-20 sm:px-6">
        <div className="w-full rounded-3xl border border-white/10 bg-white/[0.025] p-10 text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white">Project data unavailable</p>
          <h1 className="mt-5 text-3xl font-semibold tracking-[-0.04em] text-white">Internship Systems Suite could not load.</h1>
          <a href="/#projects" className="mt-7 inline-flex min-h-11 items-center justify-center rounded-full bg-neutral-100 px-5 py-3 text-sm font-semibold text-neutral-950">Return to projects</a>
        </div>
      </section>
    </CaseStudyLayout>
  );
...
export default function InternshipSystemsCaseStudyV2() {
  ...
  return (
    <CaseStudyLayout projectLabel="Internship Systems Case Study">
      {/* Scroll progress bar */}
      <motion.div aria-hidden="true" style={{ scaleX: smoothProgress }} className="fixed inset-x-0 top-0 z-[100] h-[2px] origin-left bg-gradient-to-r from-neutral-300 via-white to-neutral-400 shadow-[0_0_16px_rgba(255,255,255,0.18)]" />
        ...
        <CaseStudyHero
          ...
          media={
            <motion.div initial={reduced ? false : { opacity: 0, x: 34, scale: 0.94, filter: "blur(7px)" }} animate={{ opacity: 1, x: 0, scale: 1, filter: "blur(0px)" }} transition={{ duration: 0.9, delay: reduced ? 0 : 0.15, ease: EASE }} className="relative">
              <motion.div aria-hidden="true" initial={{ x: "-120%" }} animate={{ x: "150%" }} transition={{ duration: 1.4, delay: reduced ? 0 : 0.6, ease: EASE }} className="pointer-events-none absolute inset-y-0 left-0 z-20 w-1/3 bg-gradient-to-r from-transparent via-white/10 to-transparent blur-xl" />
              ...
              <motion.div initial={reduced ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: reduced ? 0 : 0.65, ease: EASE }} className="mt-4 flex flex-wrap gap-2">
                {["React.js", "Node.js", "Express.js", "MySQL", "Tailwind CSS"].map((t) => (
                  <span key={t} className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-xs font-semibold text-neutral-400">
                    <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-neutral-100/70" />{t}
                  </span>
                ))}
              </motion.div>
            </motion.div>
      ...
      <CaseStudySection id="problem" headingId="problem-heading" eyebrow="The Problem" title="Operations were running, but nothing was trackable." tone="elevated">
        <Reveal reduced={reduced}>
          <div className="mx-auto max-w-3xl">
            <p className="text-lg leading-9 text-neutral-300">{internshipSystems.problem}</p>
      ...
      <CaseStudySection id="system" headingId="system-heading" eyebrow="The System" title="Four applications. Each scoped to one operational domain." description="Rather than a single monolithic system, each workflow got its own focused application — sharing a consistent stack and deployment target." tone="subtle">
        <Reveal reduced={reduced}>
          <div className="flex flex-wrap gap-3">
            {systems.map((s, i) => (
              <motion.a key={s.id} href="#features" initial={reduced ? false : { opacity: 0, x: -16 }} whileInView={reduced ? undefined : { opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: reduced ? 0 : i * 0.08, ease: EASE }} className="group inline-flex items-center gap-2.5 rounded-full border border-white/10 bg-white/[0.025] px-4 py-2.5 text-sm font-medium text-neutral-200 transition duration-300 hover:border-neutral-100/30 hover:bg-neutral-100/[0.06] hover:text-white">
                <span className="font-mono text-[0.65rem] text-neutral-600">{String(i + 1).padStart(2, "0")}</span>
                {s.shortName ?? s.name}
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2 lg:items-start">
          <Reveal reduced={reduced} delay={0.1}>
            <div className="flex flex-col gap-6">
              <p className="text-sm leading-8 text-neutral-400">All four systems share a <strong className="font-semibold text-neutral-200">React + Node.js + Express + MySQL</strong> stack. Interfaces are component-driven and styled with Tailwind CSS. Each system exposes a REST API consumed by its frontend, with auth middleware and operational logic scoped per domain.</p>
              <p className="text-sm leading-8 text-neutral-400">The systems were built sequentially during the internship, with each one deployed for actual staff use before development moved to the next. That constraint shaped a focus on shipping working software over building infrastructure.</p>
            </div>
          </Reveal>
          ...
          <div className="flex flex-col gap-6">
            <Reveal reduced={reduced} delay={0.1}>
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                <p className="text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-neutral-500">Technology Stack</p>
                <div className="mt-4">
            ...
            <Reveal reduced={reduced} delay={0.18}>
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                <p className="text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-neutral-500">Module Structure</p>
                <div className="mt-4 flex flex-col gap-2 font-mono text-xs">
                  {[
                    { label: "React Frontend",   desc: "UI · State · Routing",   color: "primary"   },
                    { label: "Express REST API",  desc: "Auth · Business Logic",  color: "secondary" },
                    { label: "MySQL Database",    desc: "Per-domain schema",      color: "tertiary" },
                  ].map((mod, i) => (
                    <motion.div key={mod.label} initial={reduced ? false : { opacity: 0, x: 12 }} whileInView={reduced ? undefined : { opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: reduced ? 0 : 0.2 + i * 0.08, ease: EASE }} className={["flex items-center justify-between rounded-xl border px-3.5 py-2.5", mod.color === "primary" ? "border-neutral-100/20 bg-neutral-100/[0.04]" : "", mod.color === "secondary" ? "border-neutral-500/20 bg-neutral-500/[0.04]" : "", mod.color === "tertiary" ? "border-neutral-700/25 bg-neutral-800/[0.08]" : ""].join(" ")}>
                      <span className={["font-semibold", mod.color === "primary" ? "text-white" : "", mod.color === "secondary" ? "text-neutral-300" : "", mod.color === "tertiary" ? "text-neutral-400" : ""].join(" ")}>{mod.label}</span>
                      <span className="text-neutral-600">{mod.desc}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
      ...
      <CaseStudySection id="outcome" headingId="outcome-heading" eyebrow="Outcome" title="What the work demonstrates." tone="elevated">
        <div className="grid gap-8 lg:grid-cols-2 lg:items-start">
          <Reveal reduced={reduced}>
            <p className="text-lg leading-9 text-neutral-300">{internshipSystems.outcome}</p>
            <div className="mt-8 flex flex-col gap-3">
              {[
                ...
                <motion.div key={item.eyebrow} initial={reduced ? false : { opacity: 0, x: -18 }} whileInView={reduced ? undefined : { opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.55, delay: reduced ? 0 : i * 0.09, ease: EASE }} className="flex items-start gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                  <CheckDot />
                  <div>
                    <p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-white">{item.eyebrow}</p>
                    <p className="mt-1.5 text-sm leading-7 text-neutral-400">{item.text}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <Reveal reduced={reduced} delay={0.2}>
              <div className="relative overflow-hidden rounded-2xl border border-neutral-100/15 bg-neutral-100/[0.04] p-5">
                <motion.div aria-hidden="true" initial={reduced ? false : { scaleX: 0 }} whileInView={reduced ? undefined : { scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1.1, delay: 0.3, ease: EASE }} className="absolute inset-x-0 top-0 h-px origin-left bg-gradient-to-r from-neutral-100 via-white/40 to-transparent" />
                <p className="text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-white">Confidentiality</p>
                <p className="mt-3 text-sm leading-7 text-neutral-400">These systems were built for internal operational use. The case study focuses on architecture, workflow design, interface behavior, and technical contribution without exposing private company records, credentials, or operational data.</p>
              </div>
            </Reveal>
          </div>
        </div>
        <Reveal reduced={reduced} delay={0.15} className="mt-14">
          ...
          <div className="mt-5">
            <a href="/#contact" className="inline-flex min-h-12 items-center justify-center rounded-full border border-white/15 bg-white/[0.035] px-6 py-3 text-sm font-semibold text-white transition hover:border-white/40 hover:bg-neutral-100/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-4 focus-visible:ring-offset-neutral-950">
              Discuss a project
            </a>
          </div>
        </Reveal>
